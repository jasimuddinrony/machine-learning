...
class Classifier:
    ...
    def getPrediction(self, img, draw=True, pos=(50, 50), scale=2, color=(0, 255, 0), not_match_label="Not Match", threshold=0.6):
        ...
        print("prediction value is: ", prediction[0][indexVal])
        

        if draw and self.labels_path:
            if prediction[0][indexVal] < threshold:  # Set a threshold for prediction confidence
                label = not_match_label
                print("prediction label is: ", label)
            else:
                label = self.list_labels[indexVal]
                print("prediction label is: ", label)
            cv2.putText(img, str(label), pos, cv2.FONT_HERSHEY_COMPLEX, scale, color, 2)

        return list(prediction[0]), indexVal, label
